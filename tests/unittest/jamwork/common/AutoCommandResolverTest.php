...
class AutoCommandResolverTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$this->eventDispatcher->expects($this->exactly(3))->method('triggerEvent')->will($this->returnValue($this->event));

		$this->event->expects($this->exactly(3))->method('isCanceled')->will($this->returnValue(false));
