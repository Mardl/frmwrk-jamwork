 ...
 */
class MssqlQuery implements Query
{
	 ...
	 * @param string         $concat
	 * @return $this|MssqlQuery|Query|string
	...
	{
		if (!($value instanceof MssqlQuery))
		{
			throw new \Exception('InnerStatement muss MssqlQuery Objekt sein!');
		}
	...
	/**
	 * @param string                     $field
	 * @param \jamwork\database\MssqlQuery $query
	 * @param bool                       $positiv
	 * @return string
	 */
	private function innerStatementEx($field, MssqlQuery $query, $positiv = true)
	{
		$innerStmt = $query->get();
	 ...
	 * @param string           $op
	 * @param string           $concat
	 * @return MssqlQuery|Query|string
	 * @throws \InvalidArgumentException
	 */
	...
	{
		/** In mssql kann es nur ein Limit geben, Offset wird nicht unterstützt */
		$this->limit = array();
		if ($limit !== null)
		{
			$this->limit[] = $limit;
